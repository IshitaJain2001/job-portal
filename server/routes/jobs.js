...
// Get current user's posted jobs (MUST BE BEFORE /:id route)
router.get('/my-jobs', auth, async (req, res) => {
  try {
    console.log('🔍 Fetching jobs for user:', req.userId);
...
router.get('/', async (req, res) => {
  try {
    const { category, location, search, companyCategory } = req.query;
    ...
    const jobs = await Job.find(query)
      .populate('postedBy', 'name companyName email companyCategory isVerified')
      .sort({ createdAt: -1 });

    // Filter by company category if specified
    let filteredJobs = jobs;
    if (companyCategory) {
      filteredJobs = jobs.filter(job => {
        return job.postedBy?.companyCategory === companyCategory;
      });
    }
...
router.get('/:id', async (req, res) => {
  try {
    const job = await Job.findById(req.params.id)
      .populate('postedBy', 'name companyName email companyCategory isVerified companyDescription companyWebsite')
